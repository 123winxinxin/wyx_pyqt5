...
<img src="https://img-blog.csdn.net/20161026164739484" alt="unittestç±»å¾" style="zoom:80%;" />
...
skip装饰器一共有三个 `unittest.skip(reason)`、`unittest.skipIf(condition, reason)`、`unittest.skipUnless(condition, reason)`，skip无条件跳过，skipIf当condition为`True`时跳过，skipUnless当condition为`False`时跳过。
...
`HTMLTestRunner`是一个第三方的unittest HTML报告库，首先我们下载`HTMLTestRunner.py`，并放到当前目录下，或者你的`C:\Python27\Lib`下，就可以导入运行了。

[官方链接][**http://tungwaiyip.info/software/HTMLTestRunner.html**]
...
<img src="https://img-blog.csdn.net/20161027113650705" alt="report.png" style="zoom:100%;" />
